...
class SliderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $rules = [
               ...
               'photo_ar'=>'required|mimes:jpeg,jpg,png,svg'
                ];
        ...
        $photo_ar = $name;
        $subtitle_text_ar = !empty($request->subtitle_text_ar)?$request->subtitle_text_ar:null;
        $title_text_ar =!empty($request->title_text_ar)?$request->title_text_ar:null;
        $details_text_ar = !empty($request->details_text_ar)?$request->details_text_ar:null;
        $ar_data  = ['id'=>$data->id,'details_text'=>$details_text_ar,'title_text'=>$title_text_ar,'subtitle_text'=>$subtitle_text_ar,'photo'=>$photo_ar];
        $data_ar = Slider_ar::create($ar_data);
        $data_ar->save();
    ...
    public function destroy($id)
    {
        ...
        $data->delete();
         ...
         //Ar Slider Section
        $data_ar = Slider_ar::findOrFail($id);
        if (file_exists(public_path().'/assets/images/sliders/'.$data_ar->photo)) {
            unlink(public_path().'/assets/images/sliders/'.$data_ar->photo);
        }
        $data_ar->delete();
        //--- Redirect Section     
        $msg = 'Data Deleted Successfully.';
